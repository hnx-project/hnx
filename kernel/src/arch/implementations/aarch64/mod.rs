...
use crate::arch::traits::exception::{
    BreakpointHandle, BreakpointKind, ExceptionAction, ExceptionHandler, ExceptionVector,
};
...
impl CpuArch for Aarch64Arch {
    ...
    fn start_secondary(cpu_id: usize, entry_point: usize) -> ArchResult<()> {
        cpu::start_secondary(cpu_id, entry_point)
    }
    ...
    fn restore_interrupts(previous_state: bool) {
        cpu::restore_interrupts(previous_state)
    }
...
impl MmuArch for Aarch64Arch {
    ...
    fn current_page_table() -> Self::PageTable {
        mmu::current_page_table()
    }
    fn switch_page_table(table: &Self::PageTable) {
        mmu::switch_page_table(table)
    }
    fn map(
...
impl InterruptArch for Aarch64Arch {
    ...
    fn register_handler(
        ...
        interrupt::register_handler(irq, handler, data)
    }
...
impl TimerArch for Aarch64Arch {
    ...
    fn register_callback(callback: TimerCallback, data: usize) -> ArchResult<TimerHandle> {
        timer::register_callback(callback, data)
    }
    fn unregister_callback(handle: TimerHandle) -> ArchResult<()> {
        timer::unregister_callback(handle)
    }
    fn set_oneshot(
        timer: &Self::Timer,
        nanoseconds: u64,
        ...
        timer::set_oneshot(timer, nanoseconds, callback, data)
    }
    fn set_periodic(
        timer: &Self::Timer,
        period_ns: u64,
