    ...
    match num {
        ...
        HNX_SYS_IPC_WAIT => {
            let _ = crate::process::block_process(
                crate::core::scheduler::current_pid() as usize
            );
            0
        }
        HNX_SYS_IPC_WAKE => {
...
fn sys_write(fd: usize, buf: usize, len: usize) -> SysResult {
    ...
    if let Some(ref mut e) = tbl[fd] {
        ...
        let r = crate::core::ipc::call(crate::core::ipc::Msg {
            service: crate::core::ipc::SERVICE_VFS,
            op: 8,  // VFS_OP_WRITE
            p1: key.as_ptr(),
            l1: pos,
...
fn sys_exit(_status: usize) -> SysResult {
    ...
    crate::core::scheduler::exit_current()
}

fn sys_getpid() -> SysResult {
    let r = crate::core::ipc::call(crate::core::ipc::Msg {
        service: crate::core::ipc::SERVICE_PROC,
        op: 1,
        p1: core::ptr::null(),
        l1: 0,
...
fn sys_open(path_user: usize, flags: u32, mode: u32) -> SysResult {
    ...
    if !file_exists && should_create {
        let create_r = crate::core::ipc::call(crate::core::ipc::Msg {
            service: crate::core::ipc::SERVICE_VFS,
            op: 9,  // VFS_OP_CREATE
            p1: p.as_ptr(),
            l1: p.len(),
    ...
    // Get file size
    let sz = crate::core::ipc::call(crate::core::ipc::Msg {
        service: crate::core::ipc::SERVICE_VFS,
        op: 7,
        p1: p.as_ptr(),
        l1: p.len(),
    ...
    for fd in 3..tbl.len() {
        if tbl[fd].is_none() {
            ...
            if writable && (flags & O_TRUNC) != 0 {
                let _ = crate::core::ipc::call(crate::core::ipc::Msg {
                    service: crate::core::ipc::SERVICE_VFS,
...
fn sys_ep_create() -> SysResult {
    ...
    let capabilities = crate::core::ipc::EndpointCapabilities {
        read: true,
        write: true,
        admin: true,
    };
    
    match crate::core::ipc::endpoint_create(capabilities) {
        Ok(ep) => {
...
fn sys_ep_send(handle: usize, op: usize, buf_user: usize, len: usize) -> SysResult {
    ...
    let src = crate::core::scheduler::current_pid() as u32;
    ...
    let msg = crate::core::ipc::IpcMessage {
        ...
        op: (op & 0xFFFF) as u16,
        priority: crate::core::ipc::Priority::Normal,
        data_len: cn,
        data: k,
    ...
    if let Some(cap) = caps_lookup(pid, handle) {
        ...
        // Send using new API
        match crate::core::ipc::endpoint_send_sync(cap.ep_id, msg) {
...
fn sys_ep_recv(handle: usize, buf_user: usize, len: usize) -> SysResult {
    ...
    if let Some(cap) = caps_lookup(pid, handle) {
        ...
        match crate::core::ipc::endpoint_recv_sync(cap.ep_id, None) {
            Ok(msg) => {
...
fn sys_mmap(addr: usize, length: usize, prot: usize, flags: usize, fd: usize, offset: usize) -> SysResult {
    ...
    let map_type = if (flags & 0x20) != 0 { // MAP_ANONYMOUS
        crate::memory::MemoryMapType::Anonymous
    } else {
        crate::memory::MemoryMapType::File
    };
    ...
    let map_flags = crate::memory::MemoryMapFlags::new(
        readable, 
        writable, 
        executable, 
    ...
    let mut entry = crate::memory::MemoryMapEntry::new(
        effective_addr, 
        aligned_length, 
        map_type, 
        map_flags
    ...
    if map_type == crate::memory::MemoryMapType::File {
...
fn sys_mprotect(addr: usize, length: usize, prot: usize) -> SysResult {
    ...
    let new_flags = crate::memory::MemoryMapFlags::new(
        readable, 
        writable, 
        executable, 
