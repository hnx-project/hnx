...
pub fn init() {
    crate::info!("network/tcp: initializing");
}
...
pub fn process_packet(interface_id: u32, src_ip: u32, data: &[u8]) -> Result<()> {
    ...
    crate::info!("network/tcp: received packet from port {} to port {}", 
                 src_port, dst_port);
    ...
    crate::info!("network/tcp: flags: {}", flag_str);
...
fn process_tcp_packet(interface_id: u32, src_ip: u32, src_port: u16, dst_port: u16, 
    ...
    if let Some(socket) = sockets.get_mut(&dst_port) {
        match socket.state {
            ...
            _ => {
                // Handle other states
                crate::info!("network/tcp: unhandled state {:?}", socket.state);
            }
        }
    } else {
        crate::info!("network/tcp: no socket bound to port {}", dst_port);
...
fn handle_syn_packet(interface_id: u32, src_ip: u32, src_port: u16, dst_port: u16, 
    ...
    crate::info!("network/tcp: handling SYN packet from {}:{}", 
                 src_ip, src_port);
    ...
    if pending_connections.len() >= backlog {
        crate::info!("network/tcp: connection queue is full, dropping SYN packet");
    ...
    crate::info!("network/tcp: added connection to pending queue, queue size: {}", 
...
fn handle_data_packet(interface_id: u32, src_ip: u32, src_port: u16, dst_port: u16, 
    ...
    crate::info!("network/tcp: handling data packet");
...
pub fn accept(listen_port: u16) -> Result<(u32, u16)> {
    ...
    crate::info!("network/tcp: accepting connection from {}:{}", 
...
mod tests {
    ...
    #[test]
    fn test_send_recv_functions_exist() {
        // This test just verifies that the send_to and recv_from functions exist
        // and can be compiled. Actual functionality would be tested in integration tests.
