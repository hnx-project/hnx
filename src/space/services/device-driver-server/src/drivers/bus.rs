//! Bus probing implementation for user-space device driver server
//!
//! This module implements bus probing functionality that communicates with the kernel
//! through IPC to handle device detection and enumeration.
...
/// Convert big endian u64 to native endian
fn be64(x: u64) -> u64 { 
    u64::from_be(x) 
}

/// Detect devices on the bus using device tree blob
            ...
            if token == 1 {
                ...
                if pname == "compatible" {
                    ...
                    if compat_virtio && len >= 8 {
                        ...
                        if other_pos < others.len() { 
                            others[other_pos] = Some(b as usize); 
                            other_pos += 1; 
                        }
                    }
                }
...
/// Architecture type for device detection
